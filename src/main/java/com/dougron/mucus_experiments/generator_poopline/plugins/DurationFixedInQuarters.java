...
public class DurationFixedInQuarters extends PlugGeneric implements PooplinePlugin {

//	public enum EndNoteDurationSolution {LEGATO_TILL_END_OF_BAR, STACCATO}
	
	public static final Logger logger = LogManager.getLogger(DurationFixedInQuarters.class);
	
	private DurationFixedInQuartersRepo durationQuartersRepo;
...
//	private EndNoteDurationSolution endNoteSolution = EndNoteDurationSolution.LEGATO_TILL_END_OF_BAR;
	...
	private double[] durationPattern;
...
//		double staccatoDurationInQuarters = getStaccatoDurationInQuarters(pack);
	...
	{
		durationQuartersRepo = DurationFixedInQuartersRepo.builder()
				.durationPattern(durationPattern)
				.className(getClass().getName())
				.build();
		pack.getRepo().put(Parameter.DURATION, durationQuartersRepo);
		return pack;
	}
	...
	{
		durationQuartersRepo = (DurationFixedInQuartersRepo)pack.getRepo().get(Parameter.DURATION);
		durationPattern = durationQuartersRepo.getDurationPattern();
	}
...
//	}
	...
	private void processMuDurations(PooplinePackage pack) {
		List<Mu> muList = pack.getMu().getMusWithNotes();
		Collections.sort(muList, Mu.globalPositionInQuartersComparator);
		int index = 0;
		for (Mu mu: muList)
		{
			mu.setLengthInQuarters(durationPattern[index]);
			index++;
			if (index >= durationPattern.length) index = 0;
		}
...
//
//
//	public DurationFixedInQuarters setStaccatoDurationInMilliseconds(int staccatoDurationInMilliseconds) {
...
//
//
//	public DurationFixedInQuarters setEndNoteSolution(EndNoteDurationSolution endNoteSolution) {
