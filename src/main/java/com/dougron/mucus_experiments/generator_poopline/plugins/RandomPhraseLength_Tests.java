...
class RandomPhraseLength_Tests {

	@Test
	void instantiates() {
		PooplinePlugin plug = new RandomPhraseLength();
	}
	...
	@Test
	void when_passing_a_blank_package_to_RandomPhraseLength_then_json_PHRASE_LENGTH_will_have_random_seed_withTestRandom_value_and_type_double() throws Exception {
		PooplinePackage pack = new PooplinePackage("x", new TestRandom(0.1));
		PooplinePlugin plug = new RandomPhraseLength();
		pack = plug.process(pack);
		JsonObject json = pack.getJson();
	...
	@Test
	void when_passing_a_blank_package_to_RandomPhraseLength_then_json_PHRASE_LENGTH_will_have_options_values_equal_to_RandomPhraseLength_options_and_type_int_array() throws Exception {
		PooplinePackage pack = new PooplinePackage("x", new TestRandom(0.1));
		PooplinePlugin plug = new RandomPhraseLength();
		pack = plug.process(pack);
		JsonObject json = pack.getJson();
		...
		int[] optionsArr = ((RandomPhraseLength)plug).getOptions();
	...
	@Test
	void when_passing_a_blank_package_to_RandomPhraseLength_then_json_PHRASE_LENGTH_will_have_selected_option_equal_to_4() throws Exception {
		PooplinePackage pack = new PooplinePackage("x", new TestRandom(0.2));
		PooplinePlugin plug = new RandomPhraseLength();
		pack = plug.process(pack);
		JsonObject json = pack.getJson();
		JsonObject phraseLength = json.get("PHRASE_LENGTH").getAsJsonObject();
		assertThat(phraseLength.get("selected_option").getAsInt()).isEqualTo(4);
	}
	
	
	@Test
	void when_passing_a_blank_package_to_RandomPhraseLength_then_json_PHRASE_LENGTH_will_have_plug_in_class_name_same_as_class_name_for_RandomPhraseLength() throws Exception {
		PooplinePackage pack = new PooplinePackage("x", new TestRandom(0.2));
		PooplinePlugin plug = new RandomPhraseLength();
		pack = plug.process(pack);
		JsonObject json = pack.getJson();
