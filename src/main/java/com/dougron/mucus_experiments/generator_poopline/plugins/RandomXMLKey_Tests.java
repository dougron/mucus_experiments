...
class RandomXMLKey_Tests {

	@Test
	void instantiates() {
		PooplinePlugin plug = new RandomXMLKey();
	}
	...
	void when_passing_a_blank_package_to_RandomXMLKey_then_json_XMLKEY_will_have_random_seed_withTestRandom_value_and_type_double() throws Exception {
		...
		pack = plug.process(pack);
		JsonObject json = pack.getJson();
		JsonObject phraseLength = json.get("XMLKEY").getAsJsonObject();
	...
	void when_passing_a_blank_package_to_RandomXMLKey_then_json_XMLKEY_will_have_options_values_equal_to_RandomXMLKey_options_and_type_int_array() throws Exception {
		...
		int[] optionsArr = ((RandomXMLKey)plug).getOptions();
...
//		random_seed remains the same
		JsonObject randomSeed = pack.getJson().get("XMLKEY").getAsJsonObject().get("random_seed").getAsJsonObject();
