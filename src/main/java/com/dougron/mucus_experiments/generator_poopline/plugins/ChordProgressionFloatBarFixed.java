...
public class ChordProgressionFloatBarFixed extends PlugGeneric implements PooplinePlugin {

	
	public static final Logger logger = LogManager.getLogger(ChordProgressionFloatBarFixed.class);
...
//	Type listDoubleType = new TypeToken<List<Double>>() {}.getType();
	...
	public ChordProgressionFloatBarFixed() {
		super(
	...
	PooplinePackage makeRepo(PooplinePackage pack)
	{
		if (hasRequiredRepos())
		{
			// currently does not generate a repo, requires one to exist already
			logger.debug("Repo asked for but not made as this plugin currently does not make plugins.");
